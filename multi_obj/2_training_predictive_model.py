...
def write_training_report(all_results, runtime_params, output_path):
    ...
    model_labels = {
        ...
        "sigmoid_mm": "Model 2 (sigmoid_mm - Regressor)",
    }
    ...
    for i, (case_study, case_results) in enumerate(all_results.items(), start=1):
        ...
        lines.append("-" * 70)
        lines.append(f"* Optuna trials: {runtime_params['optuna_trials']} | Early stopping rounds: {runtime_params['early_stopping_rounds']}")
        lines.append("* Search space:")
        lines.append(_format_search_space(runtime_params["search_spaces"]))
        lines.append("")
