...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Option choice to also simulate the baseline (no recommendation) scenario.",
        )
    parser.add_argument(
...
def main():
    ...
    is_baseline = True if baseline == "baseline" else False
    ...
    res_path = case_dir / "recommendations" /f"recommendations_{case_study}_{method}"
    ...
    # NOTE: build_recommender_df mutates prev_log in place (it adds the
    # "recommendation:act"/"recommendation:res" columns and returns the same
    # object), so we take a copy now to use later for the baseline
    # simulation (no injected recommendation).
    prev_log_baseline = prev_log.copy()
    ...
    # -----------------------------------------------------------------
    # 7. Baseline simulation: same prefixes, but WITHOUT injecting any
    #    recommended next activity/resource. The simulator continues
    #    freely from the discovered process model. This is independent
    #    of the recommendation method, so it's saved once and reused.
    # -----------------------------------------------------------------
    if is_baseline:
        baseline_folder = case_dir / "prosit_simulation_results" / "baseline"
        baseline_folder.mkdir(parents=True, exist_ok=True)

        existing_baseline_runs = list(baseline_folder.glob("sim_*.csv"))
        if len(existing_baseline_runs) >= args.n_sim:
            print(
                f"Baseline simulations already exist in {baseline_folder} "
                f"({len(existing_baseline_runs)} files found >= n_sim={args.n_sim}). Skipping."
            )
        else:
            print("Building baseline log (no recommendation forced)...")
            # IMPORTANT: SimulatorEngine.apply() uses the "recommendation:act"/
            # "recommendation:res" columns for two DIFFERENT purposes:
            #   1. To decide which cases are "prefixes" to resume simulating at
            #      all (any case with a non-null value in either column).
            #   2. To force that specific activity/resource as the next step,
            #      when it matches a currently enabled transition.
            # So we can't just omit the columns (that breaks purpose #1). Instead
            # we inject a sentinel "act" value that is guaranteed not to match
            # any real activity label in the process model. This makes every
            # case still be recognized as a prefix to continue (purpose #1),
            # while purpose #2 never triggers, since the sentinel can never be
            # an enabled transition -> the simulator falls back to its normal,
            # unforced, weighted transition choice.
            sentinel_act = f"__NO_RECOMMENDATION__{uuid.uuid4().hex}"
            baseline_recommendations = {
                case_id: {"act": sentinel_act, "res": None}
                for case_id in prev_log_baseline["case:concept:name"].unique()
            }
            log_baseline = build_recommender_df(prev_log_baseline, baseline_recommendations)
            ...
            # Apply the same optional case_ids filtering used for log_rec, so the
            # baseline is directly comparable to the recommendation simulations.
            if args.case_ids:
                print(f"Restricting log_baseline to {len(case_ids)} case ids from {args.case_ids}")
