...
#                    feature_weights[feature] = round(1 / normalized_mads[feature], 2)
        ...
        if feature_weights != self.feature_weights_input:
            ...
            for idx, feature in enumerate(self.data_interface.feature_names):
                if idx in self.data_interface.categorical_feature_indexes and feature not in feature_weights: #categorical feature
...
# Andrei's version with adding max_attempts as stopping theshold
    def do_random_init(self, num_inits, features_to_vary, query_instance, desired_class, desired_range):
        ...
        act_index = self.data_interface.feature_names.index('NEXT_ACTIVITY')
        
        attempts = 0
        while len(valid_inits) < num_inits and attempts < max_attempts:
            attempts += 1
            ...
            for jx, feature in enumerate(self.data_interface.feature_names):
                if feature == 'NEXT_ACTIVITY':
                    random_inits[:, jx] = np.random.choice(self.feature_range[feature], num_remaining)
                elif feature == 'NEXT_RESOURCE':
                    decoded_activities = [
                        self.labelencoder['NEXT_ACTIVITY'].inverse_transform([int(random_inits[:, act_index][i])])[0]
                        for i in range(num_remaining)
                    ]
                    random_inits[:, jx] = [
                        self.labelencoder['NEXT_RESOURCE'].transform([np.random.choice(self.act_res[i])])[0]
                        for i in decoded_activities
                    ]
                else:
            ...
            valid_inits.extend(random_inits[valid_mask_flat])
...
#            random_inits = np.zeros((num_remaining, num_features))
#            for jx, feature in enumerate(self.data_interface.feature_names):
##                if feature in features_to_vary: #NEXT_ACTIVITY
#                if feature == 'NEXT_ACTIVITY':
#                    random_inits[:, jx] = np.random.choice(self.feature_range[feature], num_remaining)
#                # Diem Added
#                elif feature == 'NEXT_RESOURCE':
#                        decoded_activities = [self.labelencoder['NEXT_ACTIVITY'].inverse_transform([int(random_inits[:,act_index][i])])[0] for i in range(num_remaining)]
#                        random_inits[:, jx] = [self.labelencoder['NEXT_RESOURCE'].transform([np.random.choice(self.act_res[i])])[0] for i in decoded_activities]
#                else:
#                    random_inits[:, jx] = query_instance[jx]
#
#            # Filter out the valid initializations
...
#            valid_mask_flat = valid_mask.flatten() #modified here
#            valid_inits.extend(random_inits[valid_mask_flat])
#
#        return np.array(valid_inits[:num_inits])
    ...
    def _generate_counterfactuals(self, query_instance, total_CFs, initialization="random",
                                  ...
                                  posthoc_sparsity_algorithm="binary", maxiterations=100, thresh=1e-2, verbose=False, act_res = None):
        ...
        self.population_size = 10 * total_CFs # MODIFY HERE IF NEEDED! Default = 10
        self.act_res = act_res # Diem Added
        ...
        self.data_interface.set_continuous_feature_indexes(query_instance) #Diem Added
    ...
    def _predict_fn_custom(self, input_instance, desired_class):
        ...
        if self.model.model_type == ModelTypes.Classifier and np.array(output).shape[1] == 1: #Our case shape = 2
            # DL models return only 1 for binary classification
            output = np.hstack((1-output, output))
        desired_class = int(desired_class)
        maxvalues = np.max(output, 1) #returns max value by column not row
        predicted_values = np.argmax(output, 1)
        # predicted_values = output[0][desired_class] #Getting probability of being 1 #CHANGE HERE!!!
        ...
        for i in range(len(output)):
            if output[i][desired_class] == maxvalues[i]:
               predicted_values[i] = desired_class
                # predicted_values[i] = predicted_values[i] #CHANGE HERE!!!
...
#            [self.feature_weights_list[0][i] for i in self.data_interface.continuous_feature_indexes])
        feature_weights = np.array(self.feature_weights_list[0])
        copy_cont_feature_indexes = self.data_interface.continuous_feature_indexes.copy()
#        copy_cont_feature_indexes = feature_to_vary_indexes + copy_cont_feature_indexes
        copy_cont_feature_indexes.sort()
        self.data_interface.feature_weights_indexes = copy_cont_feature_indexes
        product = np.multiply(
...
#
#                if prob < 0.80:
#                    if feat_name == 'NEXT_RESOURCE' and 'NEXT_ACTIVITY' in features_to_vary:
#                        act_val = one_init[act_index]
#
#                        # Try parent 1 gene if feasible
#                        if gp1 in self.act_res.get(act_val, []):
#                            gene_value = gp1
#                        # Else try parent 2 gene if feasible
#                        elif gp2 in self.act_res.get(act_val, []):
#                            gene_value = gp2
#                        else:
#                            # Sample feasible 'res' given 'act'
#                            gene_value = np.random.choice(self.act_res.get(act_val, self.feature_range[feat_name]))
#                    else:
#                        # Normal crossover for other features
#                        gene_value = gp1 if prob < 0.40 else gp2
#                else:
#                    # Mutation
#                    if feat_name in features_to_vary:
#                        if feat_name in self.data_interface.continuous_feature_names:
#                            gene_value = np.random.uniform(
#                                self.feature_range[feat_name][0], self.feature_range[feat_name][1]
#                            )
#                        else:
#                            if feat_name == 'NEXT_RESOURCE' and 'NEXT_ACTIVITY' in features_to_vary:
#                                act_val = one_init[act_index]
#                                if act_val in self.act_res:
#                                    gene_value = np.random.choice(self.act_res[act_val])
#                                else:
#                                    gene_value = np.random.choice(self.feature_range[feat_name])
#                            else:
#                                gene_value = np.random.choice(self.feature_range[feat_name])
#                    else:
#                        gene_value = query_instance[j]
#
#                one_init[j] = gene_value
#
#            return one_init
    ...
    def mate(self, k1, k2, features_to_vary, query_instance):
        ...
        for j in range(self.data_interface.number_of_features):
            ...
            # random probability
            prob = random.random()
            if feat_name == 'NEXT_RESOURCE':
                # Get back the name of Next Activity
                decoded_act = self.labelencoder['NEXT_ACTIVITY'].inverse_transform([int(one_init[act_index])])[0]
                decoded_gp1 = self.labelencoder['NEXT_RESOURCE'].inverse_transform([int(gp1)])[0]
                decoded_gp2 = self.labelencoder['NEXT_RESOURCE'].inverse_transform([int(gp2)])[0]
                
                if (prob < 0.40) & (decoded_gp1 in self.act_res[decoded_act]):
                    # if prob is less than 0.40, insert gene from parent 1
                    one_init[j] = gp1
                elif (prob < 0.80)  & (decoded_gp2 in self.act_res[decoded_act]):
                    # if prob is between 0.40 and 0.80, insert gene from parent 2
                    one_init[j] = gp2
                else:
#                    one_init[j] = np.random.choice(one_init[act_index])
                    one_init[j] = self.labelencoder['NEXT_RESOURCE'].transform([np.random.choice(self.act_res[decoded_act])])[0]
...
#                    new_generation_2[new_gen_idx] = child
            if rest_members > 0:
                # Create an array to store the new generation
                 new_generation_2 = np.zeros((rest_members, self.data_interface.number_of_features))

                # Randomly generate parent indices for all offspring in advance
                 high = max(1, len(population) // 2) # Using this to AVOID len(pop) = 1
                 parent_indices1 = np.random.randint(0, high, size=rest_members)
                 parent_indices2 = np.random.randint(0, high, size=rest_members)

                # Retrieve the parent individuals from the population
                 parents1 = np.array([population[idx] for idx in parent_indices1])
                 parents2 = np.array([population[idx] for idx in parent_indices2])

#                # Vectorized mating to create offspring
#                if adapted:
#                    new_generation_2 = np.array([
#                        self.mate_2(parents1[i], parents2[i], features_to_vary, query_instance,
#                                    encoder, d4py, activities, activations, targets)
#                        for i in range(rest_members)
#                    ])
#                    new_generation_2 = new_generation_2.squeeze()
#                else:

                 new_generation_2 = np.array([
                    self.mate(parents1[i], parents2[i], features_to_vary, query_instance)
                    for i in range(rest_members)
                ])
            ...
            if self.is_cf_valid(predictions):
                ...
                if not isinstance(predictions, (np.floating, float)) and len(predictions) > 1:
                   self.cfs_preds.append(np.argmax(predictions))
                    #  self.cfs_preds.append(predictions) #CHANGE HERE!!!
                else:
